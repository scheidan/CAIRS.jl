...
function f_cov(c1::Coor, c2::Coor)
    ...
    var * exp( -alpha*(dist_spatial/l_spatial)^p -
              (1.0-alpha)*(dist_temoral/l_temporal)^p )

    # return (c1.x*c2.x) + (c1.y*c2.y) + (c1.time*c2.time)
    ## sigma^2 * (alpha*exp(-(dist_spatial/l_spatial)^p) +
    ##           (1.0-alpha)*exp(-(dist_temoral/l_temporal)^p) )

end


## -----------
## covariance (kernel) function for ( Coor, Domain )

function f_cov(c::Coor, d::Domain)
    d1 = Domain(c, Coor(0,0,0), 0.0)    # a coordimate is a domain with a zero extend
    f_cov(d1, d)
end
...
## !!!! Check nuemric of integration !!!! Variance of a domain has a hugre realtive error. Why?
